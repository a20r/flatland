...
\begin{algorithm}[ht] 
    \caption{$\Function{EXTEND($\mathcal{T}$,$q$)}$~\cite{kuffner2000rrt}}
    \label{alg:EXTEND}
    \begin{algorithmic}[1]
        \setcounter{ALC@line}{0}
        \vspace*{1mm}
...
\begin{algorithm}[ht] 
    \caption{$\Function{CONNECT($\mathcal{T}$,$q$)}$~\cite{kuffner2000rrt}}
    \label{alg:CONNECT}
    \begin{algorithmic}[1]
        \setcounter{ALC@line}{0}
        \vspace*{1mm}
        \REPEAT 
            \STATE $S \leftarrow \Function{EXTEND($\mathcal{T}$,$\;q$)};$ 
...
\begin{algorithm}[ht] 
    ...
    \begin{algorithmic}[1]
        ...
        \FOR{$k = 1 \ \TO \ K$}
            ...
            \IF {$\textbf{not} \ (\Function{EXTEND($\mathcal{T}_a$,$\;q_{rand} = Trapped$)})$}
                \IF {$ (\Function{CONNECT($\mathcal{T}_b,q_{new}$}) = Reached $}
