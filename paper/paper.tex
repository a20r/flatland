...
different methods of dimensionality reduction for motion planing. The left and
right columns show the results for RRTConnect and PRM algorithms respectively.
The first, second, and third row show the average path length, computational
time in seconds, and number of failed runs out of 100 respectively. For each
dimensionality reduction technique, there are three bars presenting the results
for 250, 300, and 350 randomly generated obstacles respectively. From the data
we can see that both planners produced similarly sized paths for each
dimensionality reduction technique, and that by not reducing the dimension, the
planner is able to find a path roughly two times shorter.

In terms of computational time, there is no significant difference between
Truncated SVD, PCA, Randomized PCA, planning in the original space when
...
exists for failure rate for planning in the reduced dimension space.
